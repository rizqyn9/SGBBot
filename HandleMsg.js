...
module.exports = HandleMsg = async (aruga, message) => {
	...
	// Filter Banned People
        ...
        switch (command) {
        //  CUSTOM COMMAND R-DEV
        // Template
        // case 'CUSTOM' :
        //     await RBot.sendText(from,'TEMPAT ISI TEXT')
        //     break

        case 'pesan' :
            await RBot.sendText(from,'OK segera dilayani')
            break
        ...
        case 'ownerbot':
            ...
            .then(() => RBot.sendText(from, 'Jika kalian ingin request fitur silahkan chat nomor owner!'))
            break
        case 'join':
            if (args.length == 0) return RBot.reply(from, `Jika kalian ingin mengundang bot kegroup silahkan invite atau dengan\nketik ${prefix}join [link group]`, id)
            let linkgrup = body.slice(6)
            let islink = linkgrup.match(/(https:\/\/chat.whatsapp.com)/gi)
            let chekgrup = await RBot.inviteInfo(linkgrup)
            if (!islink) return RBot.reply(from, 'Maaf link group-nya salah! silahkan kirim link yang benar', id)
            if (isOwnerBot) {
                await RBot.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          await RBot.sendText(from, 'Berhasil join grup via link!')
                          await RBot.sendText(chekgrup.id, `Hai minna~, Im Aruga BOT. To find out the commands on this bot type ${prefix}menu`)
                      })
            } else {
                let cgrup = await RBot.getAllGroups()
                if (cgrup.length > groupLimit) return RBot.reply(from, `Sorry, the group on this bot is full\nMax Group is: ${groupLimit}`, id)
                if (cgrup.size < memberLimit) return RBot.reply(from, `Sorry, BOT wil not join if the group members do not exceed ${memberLimit} people`, id)
                await RBot.joinGroupViaLink(linkgrup)
                      .then(async () =>{
                          await RBot.reply(from, 'Berhasil join grup via link!', id)
                      })
                      .catch(() => {
        ...
        case 'stiker':
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                .then(() => {
                    RBot.reply(from, 'Here\'s your sticker')
                ...
                if (isMedia || isQuotedImage) {
                    ...
                    console.log(err)
	   	            await RBot.reply(from, 'Maaf batas penggunaan hari ini sudah mencapai maksimal', id)
                    }
                }
            } else if (args.length === 1) {
                if (!isUrl(url)) { await RBot.reply(from, 'Maaf, link yang kamu kirim tidak valid.', id) }
                RBot.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ? RBot.sendText(from, 'Maaf, link yang kamu kirim tidak memuat gambar.')
                    : RBot.reply(from, 'Here\'s your sticker')).then(() => console.log(`Sticker Processed for ${processTime(t, moment())} Second`))
            } else {
                await RBot.reply(from, `Tidak ada gambar! Untuk menggunakan ${prefix}sticker\n\n\nKirim gambar dengan caption\n${prefix}sticker <biasa>\n${prefix}sticker nobg <tanpa background>\n\natau Kirim pesan dengan\n${prefix}sticker <link_gambar>`, id)
            }
            break
        case 'stickergif':
        case 'stikergif':
            if (isMedia || isQuotedVideo) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    var mediaData = await decryptMedia(message, uaOverride)
                    RBot.reply(from, '[WAIT] Sedang di proses⏳ silahkan tunggu ± 1 min!', id)
                    ...
                    RBot.reply(from, `[❗] Kirim gif dengan caption *${prefix}stickergif* max 10 sec!`, id)
                   }
                } else {
        ...
        case 'stickergiphy':
            ...
            if (isGiphy) {
                ...
                .catch(() => {
                    RBot.reply(from, `Ada yang error!`, id)
                })
            } else {
                await RBot.reply(from, 'Maaf, command sticker giphy hanya bisa menggunakan link dari giphy.  [Giphy Only]', id)
            }
            break
        case 'meme':
            if ((isMedia || isQuotedImage) && args.length >= 2) {
                ...
                RBot.sendFile(from, ImageBase64, 'image.png', '', null, true)
                    .then(() => {
                        RBot.reply(from, 'Ini makasih!',id)
                    })
                    .catch(() => {
                        RBot.reply(from, 'Ada yang error!')
                    })
            } else {
                await RBot.reply(from, `Tidak ada gambar! Silahkan kirim gambar dengan caption ${prefix}meme <teks_atas> | <teks_bawah>\ncontoh: ${prefix}meme teks atas | teks bawah`, id)
            }
            break
        case 'quotemaker':
            ...
            if (qmaker.length >= 3) {
                ...
                RBot.reply(from, 'Proses kak..', id)
                try {
                    const hasilqmaker = await images.quote(quotes, author, theme)
                    RBot.sendFileFromUrl(from, `${hasilqmaker}`, '', 'Ini kak..', id)
                } catch {
                    RBot.reply('Yahh proses gagal, kakak isinya sudah benar belum?..', id)
                }
            } else {
                RBot.reply(from, `Pemakaian ${prefix}quotemaker |isi quote|author|theme\n\ncontoh: ${prefix}quotemaker |aku sayang kamu|-aruga|random\n\nuntuk theme nya pakai random ya kak..`)
            }
            break
        case 'nulis':
        ...
        case 'alaudio':
            if (args.length == 0) return RBot.reply(from, `*_${prefix}ALaudio <nama surah>_*\nMenampilkan tautan dari audio surah tertentu. Contoh penggunaan : ${prefix}ALaudio al-fatihah\n\n*_${prefix}ALaudio <nama surah> <ayat>_*\nMengirim audio surah dan ayat tertentu beserta terjemahannya dalam bahasa Indonesia. Contoh penggunaan : ${prefix}ALaudio al-fatihah 1\n\n*_${prefix}ALaudio <nama surah> <ayat> en_*\nMengirim audio surah dan ayat tertentu beserta terjemahannya dalam bahasa Inggris. Contoh penggunaan : ${prefix}ALaudio al-fatihah 1 en`, message.id)
              ...
              break
        case 'jsolat':
            if (args.length == 0) return RBot.reply(from, `Untuk melihat jadwal solat dari setiap daerah yang ada\nketik: ${prefix}jsolat [daerah]\n\nuntuk list daerah yang ada\nketik: ${prefix}daerah`, id)
            const solatx = body.slice(8)
            const solatj = await rugaapi.jadwaldaerah(solatx)
            await RBot.reply(from, solatj, id)
            .catch(() => {
                RBot.reply(from, 'Sudah input daerah yang ada dilist?', id)
            })
            break
        case 'daerah':
        ...
        case 'ytmp3':
			...
			download(`${arugaytdl3}${linkmp3}`, './media/ytmp3.mp3', (err) => {
				if (err) return RBot.reply(from, err, id)
				RBot.sendPtt(from, './media/ytmp3.mp3', id)
			})
        ...
        case 'ytmp4':
			...
			download(`${arugaytdl4}${linkmp4}`, './media/ytmp4.mp4', (err) => {
				if (err) return RBot.reply(from, err, id)
				RBot.sendFile(from, './media/ytmp4.mp4', '', ' id')
			})
	...
	case 'lirik':
        ...
        //Owner Bot
        case 'ban':
            if (!isOwnerBot) return RBot.reply(from, 'Perintah ini hanya untuk Owner bot!', id)
            if (args.length == 0) return RBot.reply(from, `Untuk banned seseorang agar tidak bisa menggunakan commands\n\nCaranya ketik: \n${prefix}ban add 628xx --untuk mengaktifkan\n${prefix}ban del 628xx --untuk nonaktifkan\n\ncara cepat ban banyak digrup ketik:\n${prefix}ban @tag @tag @tag`, id)
            if (args[0] == 'add') {
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) RBot.sendText(idk, `〘 *A R U G A  B C* 〙\n\n${msg}`)
                if (cvk.isReadOnly) RBot.sendText(idk, `〘 *A R U G A  B C* 〙\n\n${msg}`)
            }
        ...
        case 'leaveall': //mengeluarkan bot dari semua group serta menghapus chatnya
            ...
            RBot.reply(from, 'Success leave all group!', id)
            break
        case 'clearall': //menghapus seluruh pesan diakun bot
            ...
            RBot.reply(from, 'Success clear all chat!', id)
            break
        default:
            break
        }
